/**
 * seed-plumbers-nyc.js
 * Fetches real plumbers from Google Places API for New York City boroughs.
 * Run: node seed-plumbers-nyc.js [--dry-run]
 */
...
function inferSpecialties(name) {
  ...
  if (found.size === 0) {
    found.add("Drain Cleaning");
    found.add("Pipe Repair");
  }
  return Array.from(found);
}

function buildContent(name, city, specialties, address, phone, workingHours) {
  ...
  return [
    `<p>${name}, based in ${city}, NY, provides professional plumbing services including ${specs}. The team is fully licensed and insured, with technicians who arrive on time, diagnose accurately, and complete every job to code.</p>`,
    `<p>Whether it's a dripping faucet or a burst main line, ${name} handles jobs of all sizes with the same level of professionalism and attention to detail. Transparent pricing and upfront quotes mean no surprises on the final bill.</p>`,
    `<div class="spec-block"><h3>Services</h3>${specItems}</div>`,
    address ? `<p>${name} serves customers in ${city} and the surrounding New York area. Located at ${address.replace(", USA", "")}. ${phone ? `For service requests or estimates, call ${phone}.` : ""}</p>` : "",
    hoursHtml,
  ].filter(Boolean).join("\n");
}
...
async function run() {
  ...
  for (const { city, state, target } of TARGETS) {
    ...
    for (const searchType of SEARCH_TYPES) {
      ...
      for (const place of results) {
        ...
        if (exists) { console.log(`  → Skip (already exists): ${place.name}`); continue; }
        ...
        const content = buildContent(place.name, city, specialties, address, phone, workingHours);
        ...
        if (DRY_RUN) {
          console.log(`  [DRY] Would add: ${place.name} | ${specialties.join(", ")}`);
        } else {
          try {
            const post = new Post(postData);
            await post.save();
            console.log(`  ✓ Added: ${place.name} | ${specialties.join(", ")}`);
          } catch(e) {
